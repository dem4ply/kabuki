		...
		namespace side_scroll
		{
			...
			public class Test_motor_side_scroll_wall_slice
			{
				...
				public void Instanciate_scenary()
				{
					scene =
						Resources.Load(
							"_prefab/tests/chamber_wall_slice" ) as GameObject;
					scene = helper.instantiate._( scene );
					player_left = scene.transform.Find(
						"player_walk_left" ).gameObject;
					player = scene.transform.Find(
						"player" ).gameObject;
				...
				{
					Rigidbody2D rigidbody_left =
						player_left.GetComponent<Rigidbody2D>();
					Rigidbody2D rigidbody_normal =
				...
				{
					Rigidbody2D rigidbody_right =
						player_right.GetComponent<Rigidbody2D>();
					Rigidbody2D rigidbody_normal =
						player.GetComponent<Rigidbody2D>();
					yield return new WaitForSeconds( 0.3f );
				...
				public IEnumerator left_and_right_slice_should_have_same_speed()
				{
					...
					Assert.AreEqual(
						rigidbody_right.velocity.y,
						rigidbody_left.velocity.y, 0.01f );
				}
			}
		}
	}
